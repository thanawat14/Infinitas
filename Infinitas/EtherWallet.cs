...
namespace Infinitas
{
    public class EtherWallet
    {
        ...
        public EtherWallet(Wordlist wordList, WordCount wordCount, string seedPassword = null, string path = DEFAULT_PATH,
            IRandom random = null) : this(path, random)
        {
            InitialiseSeed(wordList, wordCount, seedPassword);
        }

        public EtherWallet(string words, string seedPassword, string path = DEFAULT_PATH, IRandom random = null) : this(path,
            random)
        {
            InitialiseSeed(words, seedPassword);
        }

        public EtherWallet(byte[] seed, string path = DEFAULT_PATH, IRandom random = null) : this(path, random)
        {
            Seed = seed.ToHex();
            var keyPath = new NBitcoin.KeyPath(GetMasterPath());
            _masterKey = new ExtKey(Seed).Derive(keyPath);
        }

        private EtherWallet(string path = DEFAULT_PATH, IRandom random = null)
        {
            Path = path;
